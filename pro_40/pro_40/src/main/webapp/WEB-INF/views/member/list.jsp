		...
		<tr>
			...
			<td width="80%" valign="top"><br>
				<table border="0" cellspacing="1" width="100%" align="center">
					<tr>
						<td colspan="8" align="center" valign="center" height="20"><font
							size="2" face="고딕">全体会員数: <b>${totcount }</b>名 - ${page }/
								${totpage } Pages
						</font></td>
					</tr>
					<tr bgcolor="e3e9ff">
						...
						<c:forEach var="list" items="${list }">
							<tr onMouseOver="style.backgroundColor='#D1EEEE'"
								...
								<td align="center" height="25"><font face="돋움" size="2"
									color="#000000">${listcount }</font></td>
								<font face="돋움" size="2" color="#000000">
									<td align="center" height="20"><font face="돋움" size="2"><a
											href="/member/admo?idx=${list.idx}">${list.name}</a></font></td>
					...
					<c:if test="${empty list }">
						<td style="text-align: center;" colspan="9" height="25">登録されば資料がありません。</td>
					</c:if>
					...
					<div align="center">
									...
									<div align="center">
										...
										<ul>
											<li style="padding: 0 30px;"><a href="${prev }"><img
													src="/images/i_prev.gif" style="width:30px; height:20px;"></a><a href="#">${page }</a>
													<a href="${next }"><img
													src="/images/i_next.gif"  style="width:30px; height:20px;"></a></li>
						...
						<div>
							...
							<form name="input" method="post" action="list?page=1" style="text-align:center;">
								<table style="width: 100px; align: center; display: inline;">
									...
									<tr>
										<td><select name="search">
												<option value="name"
													<c:if test="${pageVO.search =='name'}">selected</c:if>>name</option>
												<option value="userid"
													<c:if test="${pageVO.search =='userid'}">selected</c:if>>userid</option>
												<option value="c_code"
													<c:if test="${pageVO.search =='c_code'}">selected</c:if>>c_code</option>
										</select> <c:if test="${empty pageVO.key }">
												<td><input type="text" size=20 name="key"></td>
											</c:if> <c:if test="${!empty pageVO.key }">
												<td><input type="text" size=20 name="key"
													value="${pageVO.key }"></td>
											</c:if>
										<td><a href="javascript:send()"><input type="button"
												value="검색"></a></td>
									</tr>
